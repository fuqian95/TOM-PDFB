...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='Composition1k.toml')
    # Parse configuration
    ...
    image_dir = CONFIG.data.test_image
    trimap_dir = CONFIG.data.test_trimap
    output_path = CONFIG.data.test_out

    utils.make_dir(os.path.join(output_path, 'pred_alpha'))
    ...
    for i, image_name in enumerate(os.listdir(image_dir)):
        ...
        cv2.imwrite(os.path.join(output_path, 'pred_alpha', image_name), alpha_pred)
        print('[{}/{}] inference done : {}'.format(i, len(os.listdir(image_dir)), os.path.join(output_path, 'pred_alpha', image_name)))
