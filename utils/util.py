...
def get_foreground_area_tensor(trimap):
    """
    get 1-channel unknown area tensor from the 3-channel/1-channel trimap tensor
    """
    if CONFIG.model.trimap_channel == 3:
        ...
        uk = trimap.eq(1).float()
        fg = trimap.eq(0).float()
    weight = torch.zeros_like(uk)
    weight[uk == 1] = 1
    weight[fg == 1] = 1
    return weight

def get_background_area_tensor(trimap):
    """
    get 1-channel unknown area tensor from the 3-channel/1-channel trimap tensor
    """
    if CONFIG.model.trimap_channel == 3:
...
def get_foreground_tensor_from_pred(pred, rand_width=30, train_mode=True):
    ### pred: N, 1 ,H, W 
    N, C, H, W = pred.shape
    pred = F.interpolate(pred, size=(640,640), mode='nearest')
    pred = pred.data.cpu().numpy()
    fbuk_area = np.ones_like(pred, dtype=np.uint8)
    fbuk_area[pred < 1.0/255.0] = 0

    for n in range(N):
        uncertain_area_ = fbuk_area[n,0,:,:] # H, W
        if train_mode:
            width = np.random.randint(1, rand_width)
        else:
            width = rand_width // 2
        uncertain_area_ = cv2.dilate(uncertain_area_, Kernels[width])
        fbuk_area[n,0,:,:] = uncertain_area_

    weight = np.zeros_like(fbuk_area)
    weight[fbuk_area == 1] = 1
...
def get_background_tensor_from_pred(pred, rand_width=30, train_mode=True):
    ### pred: N, 1 ,H, W 
    N, C, H, W = pred.shape
    pred = F.interpolate(pred, size=(640,640), mode='nearest')
    pred = pred.data.cpu().numpy()
