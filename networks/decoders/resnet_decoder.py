...
class ResNet_D_Dec(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False):
        ...
        self.refine_OS1 = nn.Sequential(
            nn.Conv2d(32+3, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
        ...
        self.refine_fb_OS1 = nn.Sequential(
            nn.Conv2d(32 + 3 + 1 + 1 , 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, 6, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
        
        self.refine_fb_OS4 = nn.Sequential(
            nn.Conv2d(64 + 1, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32,6, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
        
        self.refine_fb_OS8 = nn.Sequential(
            nn.Conv2d(128, 32, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, bias=False),
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, 6, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2),)
...
class ResShortCut_D_Decoder(ResNet_D_Dec):
    ...
    def forward(self, x, mid_fea:List[torch.Tensor],image,is_training=True):
        ret = {}
        ...
        x_os8 = self.refine_OS8(x)
        if is_training:
            fb_os8 = self.refine_fb_OS8(x)
        
        x = self.layer3(x) + fea3
        x_os4 = self.refine_OS4(x)
        if is_training:
            fb_os4 = self.refine_fb_OS4(torch.cat((x, F.interpolate(x_os8, scale_factor=2.0, mode='bilinear', align_corners=False)), 1))
        
        x = self.layer4(x) + fea2
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.leaky_relu(x) + fea1
        alpha_os1 = self.refine_OS1(torch.cat((x, image), 1))
        
        alpha_os4 = F.interpolate(x_os4, scale_factor=4.0, mode='bilinear', align_corners=False)
        alpha_os8 = F.interpolate(x_os8, scale_factor=8.0, mode='bilinear', align_corners=False)
        if is_training:
            fb_os1 = self.refine_fb_OS1(torch.cat((x, image,alpha_os4,alpha_os8), 1))
        
        alpha_os1 = (torch.tanh(alpha_os1) + 1.0) / 2.0
        alpha_os4 = (torch.tanh(alpha_os4) + 1.0) / 2.0
        alpha_os8 = (torch.tanh(alpha_os8) + 1.0) / 2.0
        if is_training:
            f_os1 = torch.sigmoid(fb_os1[:, 0:3])
            b_os1 = torch.sigmoid(fb_os1[:, 3:6])
            
            f_os4 = torch.sigmoid(fb_os4[:, 0:3])
            b_os4 = torch.sigmoid(fb_os4[:, 3:6])
